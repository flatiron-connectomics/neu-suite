...
The dependency order is one-way — `em-blockrun ← em-volume-tools ← em-seg-morpho` — and
they depend on each other by relative `../sibling` path, so the layout matters:
...
One conda environment, `em-lib`, covers all three, installed editable.
